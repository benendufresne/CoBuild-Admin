...
import { BC_ROLES_ACCESS } from "src/app/constants/breadcrumb-routes";
import {
  ADD_MANAGE_ROLES,
  EDIT_MANAGE_ROLES,
  VIEW_MANAGE_ROLES,
...
import { MANAGE_ROLES_HEADING } from "src/app/constants/table-heading";
...
{
  addButtonText = TABLE_ADD_BUTTON_TEXT.ROLE;
  ...
  heading = [...MANAGE_ROLES_HEADING];
  @ViewChild(MatTableRenderComponent)
  tableRef!: MatTableRenderComponent;
  subscriptions: Subscription[] = [];
  tempList = [];
  placeholderText = TABLE_SEACRCH_PLACEHOLDER.ROLE_SEARCH_PLACEHOLDER;
  ...
  ngOnInit(): void {
    this._breadcrumbService.setBreadcrumb(BC_ROLES_ACCESS);
    this.getListing();
    this.createForm();
  ...
  edit(event) {
    this._router.navigate([
      EDIT_MANAGE_ROLES.fullUrl,
      this._common.encryption(event.element._id),
    ]);
  }

  add() {
    return this._router.navigate([ADD_MANAGE_ROLES.fullUrl]);
  }
  ...
  updateStatus(status) {
    const data = {
      roleId: status.data._id,
      status: status.id ==5?'DELETED':status.data["status"] === "BLOCKED" ? "UN_BLOCKED" : "BLOCKED",
    };
      ...
      .subscribe((res) => {
        this._toast.success(
          status.action == "BLOCKED"
            ? "Role has been Blocked Successfully"
            : "Role has been Active Successfully"
        );
        this.getListing();
      }));
  }
  changeStatusSelection() {
    if (
      this.filterForm.value["blocked"] &&
      this.filterForm.value["unblocked"]
    ) {
      this.filterForm.get("allStatus").setValue(true);
    } else {
      this.filterForm.get("allStatus").setValue(false);
    }
  }
  ...
  checkFilterFormKeys() {
    ...
    if (this.filterOptions["blocked"] && this.filterOptions["unblocked"]) {
      this.filterRedDot = true;
      this.checkFormValue = true;
      return;
    }
  ...
  filterApply(event: any) {
    ...
    if (event.apply) {
      ...
      this.filterOptions = this._common.changeDateFormat(
